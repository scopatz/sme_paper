...
% Text of abstract
\begin{abstract}
Matched abundance ratio cascades are a well-known way of computing concentrations
and flow rates for multicomponent enrichment.  Due to the form of the system 
of equations, these are often solved using standard constrained numerical 
optimizers.  Here a symbolic mathematics library (SymPy) is coupled with 
code generation techniques to produce a multicomponent solver.  This symbolic 
solver is demonstrated to be between 20 - 300 times faster than the numeric version.
Additionally, it is also shown to have at least the same, and 
occasionally more, accuracy as the corresponding numeric optimizer.  The 
...
differential and integration may be represented or applied as desired by the 
...
Still, code generation provides an interesting opportunity. It enables a workflow 
...
to the MARC equations solve for the independent variables simultaneously.

Alternatively, a symbolic mathematics library is able to handle expressions 
...
it offered the most flexibility in terms of code generation.
...
and substituted symbolically the the majority of the calculation.  The actual 
...
parameters. This is because the closed form representation may be substituted into 
...
to one equation (with potentially many substitutions) as a function of one unknown.
...
of equations is distinct from the normal mechanisms used when engineering numeric 
solutions.
...
operations in the resultant expression after their application (e.g. the chain rule).
...
which must be performed to compute a complex expression.  Hence much of the variable
...
a complex solution.  (b): The equation from (a) as a dashed black line overlaid on top
of Figure \ref{np_constraint_fig}.  The solid black line is a linearly interpolated
contour for $f(N_P, M^*)=0$. (a) \& (b) show that $N_P(M^*)$ is an accurate 
approximation near the initial guesses $N_P^0$ and $M^*=(M_j+M_k)/2$, but less 
...
on its own and (b) overlaid onto the above heat map.  This shows that the 
...
minimization performed with SymPy would have unnecessarily large overhead and would
...
cycles on the processor they take.  For example, most arithmetic operations take
...
General CSE algorithms are much more complex than this demonstration.  They are
